...
double d_mse_loss_b(Regression r) {
  double dloss = 0;
  for (size_t i = 0; i < r.N; ++i) dloss += r.w * r.xs[i] + r.b - r.ys[i];
...
void draw_axis(void) {
  ...
  // Y axis
  DrawLineEx((Vector2){MARGIN, MARGIN}, (Vector2){MARGIN, HEIGHT - MARGIN}, 3.0,
             AXIS_COLOR);
...
void draw_stats(Regression r) {
  Rectangle rect = (Rectangle){WIDTH - 2 * MARGIN - 75,
                               HEIGHT - 2 * MARGIN - 75, 75.0f, 75.0f};
  ...
  DrawText(TextFormat("b: %.2f", r.b), 5 + rect.x, 5 + rect.y + 20 * 1, 14,
           BLACK);
  DrawText(TextFormat("loss: %.2f", mse_loss(r)), 5 + rect.x,
           5 + rect.y + 20 * 2, 14, BLACK);
  EndScissorMode();
}
  ...
  while (!WindowShouldClose()) {
    BeginDrawing();
    {
      ...
      if (!started) {
        ...
        if (GetKeyPressed() == KEY_SPACE) started = true;
      } else {
        regression_step(&r);
      }
    }
